...
loglik_EM <- function(para, Y, X, Omg, n){
  ...
  for (i in 1:N) {
    for (j in (cn[i]+1):cn[i+1]) {
      z <- z + sum(Omg[i,]*log(sapply(b, lik_ijk, y=Y[j,], x=X[j,], bta1=bta1, bta2=bta2, bta3=bta3, phi=phi, rho=rho)))
    }
  }
  z
}
...
loglik_np <- function(para, Y, X, n) {
  ...
  for (i in 1:N) {
    lik_i <- rep(1, K)
    for (j in (cn[i]+1):cn[i+1]) {
      lik_i <- lik_i*sapply(b, lik_ijk, y=Y[j,], x=X[j,], bta1=bta1, bta2=bta2, bta3=bta3, phi=phi, rho=rho)
    }
...
Ellik <- function(para, Y, X, Omg, n){
  ...
  for(i in 1:N){
    for(k in 1:K){
      llik_ik <- 0
      for(j in (cn[i]+1):cn[i+1]){
        llik_ik <- llik_ik + log(lik_ijk(y=Y[j,], x=X[j,], bta1=bta1, bta2=bta2, bta3=bta3, phi=phi, rho=rho, b=b[k]))
      }
...
g <- function(Y, X, para, k){
  ...
  z <- 0
  for(j in 1:dim(Y)[1]){
    z <- z + log(lik_ijk(y=Y[j,], x=X[j,], bta1=bta1, bta2=bta2, bta3=bta3, phi=phi, rho=rho, b=b[k]))
  }
  z <- z + log(pii[k])
  return(z)
}
